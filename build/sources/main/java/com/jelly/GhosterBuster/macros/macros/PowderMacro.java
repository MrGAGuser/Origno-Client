...
import com.jelly.GhosterBuster.utils.BlockUtils.Box;
...
import static com.jelly.GhosterBuster.utils.BlockUtils.BlockUtils.*;
...
public class PowderMacro extends Macro {
    ...
    private void updateState() {
        ...
        switch (currentState) {
            ...
            case TREASURE_SOLVE:
                ...
                if (GhosterBuster.config.powGreatExplorer) {
    ...
    public void onLastRender(RenderWorldLastEvent event) {
        if(rotator.rotating)
            ...
            if(GhosterBuster.config.powMiningShape == 0)
    ...
    public void onMessageReceived(String message){
        if(message.contains("You uncovered a treasure chest!")){
            ...
            GhosterBuster.config.powGreatExplorer = false;
        }
    ...
    public void onPacketReceived(Packet<?> packet){
        if(currentState != State.TREASURE_SOLVE || !(packet instanceof S2APacketParticles) || targetChest == null || GhosterBuster.config.powGreatExplorer)
            return;

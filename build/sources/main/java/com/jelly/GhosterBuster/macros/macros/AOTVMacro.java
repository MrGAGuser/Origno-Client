...
import com.jelly.GhosterBuster.utils.HypixelUtils.MineUtils;
...
public class AOTVMacro extends Macro {
    ...
    public void onEnable() {
        ...
        if (GhosterBuster.aotvWaypoints.getSelectedRoute() == null) {
            LogUtils.addMessage("No route selected!");
            this.toggle();
            return;
        }

        ArrayList<AOTVWaypointsStructs.Waypoint> Waypoints = GhosterBuster.aotvWaypoints.getSelectedRoute().waypoints;
        ...
        if (GhosterBuster.config.autoKillMobs && GhosterBuster.config.macroType == 3) {
            GhosterBuster.mobKiller.toggle();
            MobKiller.setMobsNames(false, "Yog", "Thyst", "Automaton");
            if (GhosterBuster.config.useHyperionUnderPlayer) {
                MobKiller.scanRange = Math.min(5, GhosterBuster.config.mobKillerScanRange);
            } else {
                MobKiller.scanRange = GhosterBuster.config.mobKillerScanRange;
            }
        ...
        if (event.old.getBlock() == Blocks.cobblestone) {
            if (event.update.getBlock() == Blocks.air) {
                ...
                if (wp != null) {
                    ...
                    if (GhosterBuster.config.stopIfCobblestoneDestroyed) {
                        this.toggle();
    ...
    public void onTick(TickEvent.Phase phase) {
        ...
        if (GhosterBuster.config.autoKillMobs) {
        ...
        switch (currentState) {
            ...
            case WARPING: {
                ...
                if (GhosterBuster.config.aotvTeleportThreshold > 0) {
                    if (!firstTp && !timeBetweenLastWaypoint.hasReached((long) (GhosterBuster.config.aotvTeleportThreshold * 1000))) {
                ...
                if (!rotation.rotating)
                    rotation.initAngleLock(waypoint, GhosterBuster.config.aotvCameraWaypointSpeed);
    ...
    private BaritoneConfig getAutoMineConfig() {
        return new BaritoneConfig(
                ...
                false,
                true,
                GhosterBuster.config.aotvCameraSpeed,
                GhosterBuster.config.aotvRestartTimeThreshold, //changed with config
                null,
                null,
                256,
                0

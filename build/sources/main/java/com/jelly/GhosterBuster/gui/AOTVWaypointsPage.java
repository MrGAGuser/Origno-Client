package com.jelly.GhosterBuster.gui;
...
public class AOTVWaypointsPage extends Page {
    ...
    public AOTVWaypointsPage() {
        ...
        addNewList.setClickAction(() -> {
            if (GhosterBuster.aotvWaypoints == null || GhosterBuster.aotvWaypoints.getRoutes() == null) return;

            GhosterBuster.aotvWaypoints.getRoutes().add(new AOTVWaypointsStructs.WaypointList("New Route", false, false, new ArrayList<>()));
            redrawRoutes();
        });
        importList.setClickAction(() -> {
            ...
            try {
                ...
                if (data.startsWith("#GhosterBusterWaypoint#::")) {
                    data = data.replace("#GhosterBusterWaypoint#::", "");
                    data = new String(Base64.getDecoder().decode(data));
                    AOTVWaypointsStructs.WaypointList list = GhosterBuster.gson.fromJson(data, AOTVWaypointsStructs.WaypointList.class);
                    if (list != null) {
                        list.enabled = false;
                        GhosterBuster.aotvWaypoints.getRoutes().add(list);
                        Notifications.INSTANCE.send("GhosterBuster", "Imported route " + list.name + " from GhosterBuster successfully!");
                        redrawRoutes();
                    }
                    ...
                    data = sb.toString();
                    CategoryList categoryList = GhosterBuster.gson.fromJson(data, CategoryList.class);
                    ...
                    for (WaypointCategory category : categories) {
                        ...
                        GhosterBuster.aotvWaypoints.getRoutes().add(list);
                        Notifications.INSTANCE.send("GhosterBuster", "Imported route " + list.name + " from Skytils successfully!");
                    }
                    ...
                    data = new String(Base64.getDecoder().decode(data));
                    System.out.println(data);
                    JsonElement element = GhosterBuster.gson.fromJson(data, JsonElement.class);
                    if (element instanceof JsonObject) {
                        ...
                        if (object.has("routes")) {
                            ...
                            Notifications.INSTANCE.send("GhosterBuster", "Imported routes from GhosterBuster successfully!");
                            redrawRoutes();
                        }
                    } else if (element instanceof JsonArray) {
                        JsonArray array = (JsonArray) element;
                        ArrayList<Waypoint> waypoints = GhosterBuster.gson.fromJson(element, new TypeToken<ArrayList<Waypoint>>() {
                        }.getType());
                ...
                Notifications.INSTANCE.send("GhosterBuster", "Failed to import route!");
                Notifications.INSTANCE.send("GhosterBuster", "Error message: " + e.getMessage());
                Notifications.INSTANCE.send("GhosterBuster", "Click me to copy the message.", 5000, () -> {
                    Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
    ...
    public static void redrawRoutes() {
        ...
        if (GhosterBuster.aotvWaypoints == null || GhosterBuster.aotvWaypoints.getRoutes().size() == 0) return;
        ...
        for (AOTVWaypointsStructs.WaypointList list : GhosterBuster.aotvWaypoints.getRoutes()) {
            ...
            selected.setClickAction(() -> {
                GhosterBuster.aotvWaypoints.getRoutes().forEach(waypointList -> waypointList.enabled = false);
                GhosterBuster.aotvWaypoints.getRoutes().get(GhosterBuster.aotvWaypoints.getRoutes().indexOf(list)).enabled = true;
                redrawRoutes();
            });
            ...
            expandButton.setClickAction(() -> {
                if (Objects.equals(expandButton.getText(), "Expand")) {
                    GhosterBuster.aotvWaypoints.getRoutes().forEach(waypointList -> waypointList.showCoords = false);
                    GhosterBuster.aotvWaypoints.getRoutes().get(GhosterBuster.aotvWaypoints.getRoutes().indexOf(list)).showCoords = true;
                } else if (Objects.equals(expandButton.getText(), "Hide")) {
                    GhosterBuster.aotvWaypoints.getRoutes().get(GhosterBuster.aotvWaypoints.getRoutes().indexOf(list)).showCoords = false;
                }
                redrawRoutes();
            });
            ...
            BasicButton deleteButton = new BasicButton(80, BasicButton.SIZE_36, "Delete", null, null, BasicButton.ALIGNMENT_CENTER, ColorPalette.PRIMARY_DESTRUCTIVE);
            deleteButton.setClickAction(() -> {
                GhosterBuster.aotvWaypoints.getRoutes().remove(list);
                redrawRoutes();
            });
            ...
            shareButton.setClickAction(() -> {
                String routeString = Base64.getEncoder().encodeToString(GhosterBuster.gson.toJson(list).getBytes());
                StringSelection stringSelection = new StringSelection("#GhosterBusterWaypoint#::" + routeString);
                Clipboard clipboard = Toolkit.getDefaultToolkit().getSystemClipboard();
                clipboard.setContents(stringSelection, null);
            });
    ...
    public void draw(long vg, int x, int y, InputHandler inputHandler) {
        ...
        if (GhosterBuster.aotvWaypoints == null || GhosterBuster.aotvWaypoints.getRoutes().size() == 0 || routes.size() == 0) {

...
public class MobKiller {
            ...
            case ATTACKING:
                ...
                if (GhosterBuster.config.useHyperionUnderPlayer) {
                    ...
                    if (AngleUtils.isDiffLowerThan(mc.thePlayer.rotationYaw, 89, 0.5f)) {
                        if (attackDelay.hasReached(GhosterBuster.config.mobKillerAttackDelay) && target.distance() < 6) {
                    ...
                    if (!GhosterBuster.config.customItemToKill.isEmpty()) {
                        weapon = PlayerUtils.getItemInHotbar(GhosterBuster.config.customItemToKill);
                    } else {
                    ...
                    if (!visible) {
                        ...
                        if (targeted) {
                            if (attackDelay.hasReached(GhosterBuster.config.mobKillerAttackDelay)) {
                                if (GhosterBuster.config.attackButton == 0) {
                ...
                break;
            case BLOCKED_VISION:
                if (NpcUtil.getEntityHp(target.entity) <= 0 || target.distance() > GhosterBuster.config.mobKillerScanRange) {
                    currentState = States.KILLED;
                    afterKillDelay.reset();
                    break;
                }
    ...
    public void onRenderWorldLastRotation(RenderWorldLastEvent event) {
        ...
        if (target != null && !GhosterBuster.config.useHyperionUnderPlayer) {
            int yawRotation;
            int pitchRotation;
            ...
            rotation.initAngleLock(yawRotation, pitchRotation, GhosterBuster.config.mobKillerCameraSpeed);
        } else if (target != null) {
            if (target.distance() > 5.5) return;

            rotation.initAngleLock(mc.thePlayer.rotationYaw, 89, GhosterBuster.config.mobKillerCameraSpeed);
        }
    }

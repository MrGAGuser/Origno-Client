package com.jelly.GhosterBuster.utils.BlockUtils;

import com.google.common.cache.CacheBuilder;
import com.google.common.cache.CacheLoader;
import com.google.common.cache.LoadingCache;
import com.jelly.GhosterBuster.GhosterBuster;
import com.jelly.GhosterBuster.mixins.accessors.RenderGlobalAccessor;
...
import static com.jelly.GhosterBuster.utils.PlayerUtils.AnyBlockAroundVec3;
import static com.jelly.GhosterBuster.utils.AngleUtils.*;
...
public class BlockUtils {
    ...
    public static List<BlockPos> findBlockFromState(Box searchBox, ArrayList<BlockPos> forbiddenBlockPos, int minY, int maxY, ArrayList<IBlockState> requiredBlock) {
        ...
        for (BlockPos blockPos: BlockPos.getAllInBox(from, to)) {
            if (requiredBlock.contains(BlockUtils.getBlockState(blockPos)) && new Vec3(blockPos).distanceTo(PlayerUtils.playerEyePosVec()) < 4 && !forbiddenBlockPos.contains(blockPos) && blockPos.getY() < maxY && blockPos.getY() > minY) {
                if (AngleUtils.getRotationDifference(getBlockCenter(blockPos)) <= GhosterBuster.config.playerFov * (1.0f + (4 - new Vec3(blockPos).distanceTo(PlayerUtils.playerEyePosVec())) / 5) || mc.thePlayer.rotationPitch > 75 || mc.thePlayer.rotationPitch < -75) {
    ...
    public static Vec3 getRandomVisibilityLine(BlockPos pos) {
        ...
        ArrayList<Vec3> lines = getAllVisibilityLines(pos, mc.thePlayer.getPositionVector().add(new Vec3(0, mc.thePlayer.getEyeHeight(), 0)).subtract(new Vec3(0, lowerY ? GhosterBuster.config.aotvMiningCobblestoneAccuracy : 0, 0)), lowerY);
        if (lines.isEmpty()) {
            return null;
        } else {
    ...
    public static ArrayList<Vec3> getAllVisibilityLines(BlockPos pos, Vec3 fromEye, boolean lowerY) {
        ...
        int accuracyChecks = GhosterBuster.config.aotvMiningAccuracyChecks;
        float accuracy = 1f / accuracyChecks;
        float spaceFromEdge = lowerY ? 0.1f : GhosterBuster.config.aotvMiningAccuracy;

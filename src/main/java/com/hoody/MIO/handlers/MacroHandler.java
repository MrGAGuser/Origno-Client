...
import com.jelly.GhosterBuster.macros.macros.*;
import com.jelly.GhosterBuster.render.BlockRenderer;
...
public class MacroHandler {
    ...
    public void onMessageReceived(ClientChatReceivedEvent event) {
        ...
        if (message.contains("Your pass to the Crystal Hollows will expire in 1 minute")) {
            if (GhosterBuster.config.autoRenewCrystalHollowsPass) {
    ...
    public static void startScript(Macro macro){
        if(!macro.isEnabled()) {
            ...
            macro.toggle();
            enabled = true;
            if (GhosterBuster.config.mouseUngrab) {
    ...
    public void drawAOTV(RenderWorldLastEvent event) {
        if (!SkyblockInfo.onCrystalHollows() || !(GhosterBuster.config.macroType == 3)) return;

        if (GhosterBuster.aotvWaypoints != null && GhosterBuster.aotvWaypoints.getSelectedRoute() != null && GhosterBuster.aotvWaypoints.getSelectedRoute().waypoints != null) {
            ArrayList<AOTVWaypointsStructs.Waypoint> Waypoints = GhosterBuster.aotvWaypoints.getSelectedRoute().waypoints;

            if (GhosterBuster.config.aotvHighlightRouteBlocks) {
                for (AOTVWaypointsStructs.Waypoint waypoint : Waypoints) {
                    DrawUtils.drawBlockBox(new BlockPos(waypoint.x, waypoint.y, waypoint.z), GhosterBuster.config.aotvRouteBlocksColor, 2f);
                }
                if (GhosterBuster.config.aotvShowDistanceToBlocks) {
                    for (AOTVWaypointsStructs.Waypoint waypoint : Waypoints) {
                        BlockPos pos = new BlockPos(waypoint.x, waypoint.y, waypoint.z);
                        DrawUtils.drawText("§l§3[§f " + waypoint.name + " §3]", pos.getX() + 0.5, pos.getY() + 1.5, pos.getZ() + 0.5, GhosterBuster.config.aotvShowDistanceToBlocks);
                    }
                }
            }
            if (GhosterBuster.config.aotvShowRouteLines) {
                if (Waypoints.size() > 1) {
            ...
            if (GhosterBuster.config.drawBlocksBlockingAOTV) {
                if (!blocksBlockingVision.isEmpty()) {
                    for (BlockPos bp : blocksBlockingVision) {
                        DrawUtils.drawBlockBox(bp, GhosterBuster.config.aotvVisionBlocksColor, 1f);
                    }
                }
            }
        }
    ...
    public void scanBlockingVisionBlocks() {
        ...
        if (!GhosterBuster.config.drawBlocksBlockingAOTV) return;

        if (GhosterBuster.config.macroType == 3) {
            if (GhosterBuster.aotvWaypoints != null && GhosterBuster.aotvWaypoints.getSelectedRoute() != null && GhosterBuster.aotvWaypoints.getSelectedRoute().waypoints != null) {
                ArrayList<AOTVWaypointsStructs.Waypoint> Waypoints = GhosterBuster.aotvWaypoints.getSelectedRoute().waypoints;

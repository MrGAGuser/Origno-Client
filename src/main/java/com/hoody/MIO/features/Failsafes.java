...
import com.jelly.GhosterBuster.utils.UngrabUtils;
...
import static com.jelly.GhosterBuster.handlers.MacroHandler.macros;
...
public class Failsafes {
    ...
    public void onBlockChange(BlockChangeEvent event) {
        ...
        if (GhosterBuster.config.bedrockFailsafe) {
            ...
            if (blockChangeTimer.hasReached(1000)) {
                if (changesToBedrock > GhosterBuster.config.bedrockThreshold) {
    ...
    public void onWorldChange(WorldEvent.Load event) {
        ...
        if (mc.thePlayer == null || mc.theWorld == null)
            return;
        if (!GhosterBuster.config.disableOnWorldChange) return;
    ...
    public void onTick(TickEvent.ClientTickEvent event) {
        ...
        if (GhosterBuster.config.bedrockFailsafe) {
            ...
            if (bedrockCount > GhosterBuster.config.bedrockBackupThreshold) {
                bedrockFailsafeFake(false);
                return;
            }
        }
        ...
        if (GhosterBuster.config.playerFailsafe) {
            if (PlayerUtils.isNearPlayer(GhosterBuster.config.playerRad) && someoneIsCloseTimer == null){
                someoneIsCloseTimer = new Timer();
            } else if (!PlayerUtils.isNearPlayer(GhosterBuster.config.playerRad) && someoneIsCloseTimer != null) {
                someoneIsCloseTimer = null;
            }

            if (PlayerUtils.isNearPlayer(GhosterBuster.config.playerRad) && someoneIsCloseTimer != null && someoneIsCloseTimer.hasReached(GhosterBuster.config.playerDetectionThreshold)) {
                PingAlert.sendPingAlert();
                DisableMacros();
    ...
    public void onPacket(ReceivePacketEvent event) {
        if (!GhosterBuster.config.stopMacrosOnRotationCheck) return;
        if (macros.stream().noneMatch(Macro::isEnabled)) return;
        if (mc.thePlayer == null || mc.theWorld == null) return;
    ...
    public static TrayIcon createNotification(String text, SystemTray tray) {
        TrayIcon trayIcon = new TrayIcon(new BufferedImage(1, 1, BufferedImage.TYPE_3BYTE_BGR), "GhosterBuster Failsafe Notification");
        trayIcon.setToolTip("GhosterBuster Failsafe Notification");
        try {
            tray.add(trayIcon);
        ...
        trayIcon.displayMessage("GhosterBuster - Failsafes", text, TrayIcon.MessageType.WARNING);
        return trayIcon;
    }
    ...
    public static void bedrockFailsafeFake(boolean resumeMacro) {
        ...
        fakeMovementThread = new Thread(() -> {
            if(!GhosterBuster.config.fakeMovements)
                return;
    ...
    public static void fakeMovement(boolean resumeMacro) {
        ...
        TrayIcon trayIcon;
        if(GhosterBuster.config.notifications)
